...
export default class Game extends Phaser.Scene
{
	...
	private handlePlayerTurnChanged(playerIndex:number){
		if(playerIndex==this.server?.playerIndex)
		{
			...
			const width = this.scale.width
			this.gameStateText = this.add.text(width * 0.5, 50, 'It\'s your turn')
				.setOrigin(0.5)		
		}
		else
		{
			...
			const width = this.scale.width
			this.gameStateText = this.add.text(width * 0.5, 50, 'It\'s opponent \'s turn')
				.setOrigin(0.5)	
		}
	}
	...
	private handlePlayerWon(playerIndex: number)
	{
		this.time.delayedCall(1000, () => {
			...
			{
				this.onGameOver({
				winner: this.server?.playerIndex === playerIndex?1:0
				})
			}
		})
	}
